...
#                    logger.warning('warning\tswitch packet-in overflow %d', self.overflow_cnt)
    ...
    def handle_ip(self,datapath,msg,in_port):
        ...
        if rules == None:
            self.logger.error("error\trequest rules for packet failed: %s %s",ip_src,ip_dst)
            return
        ...
        for rule in rules[1:]:
            ...
            self.add_flow(datapath,1, 100 - rules.index(rule) + 1,match,inst,ofproto.OFP_NO_BUFFER,timeout)
